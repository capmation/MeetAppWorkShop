    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-dark-700/60">
      ...
      <div class="flex items-center gap-2">
        ...
        <template v-if="callState === 'ringing'">
          <span class="flex items-center gap-1.5 text-xs text-emerald-300 animate-pulse">
          ...
          <button
            type="button"
            class="text-xs px-2.5 py-1.5 rounded-lg border border-rose-500/40 bg-rose-500/10 text-rose-300 hover:bg-rose-500/20 transition"
            @click="$emit('cancelCall')"
          >
            Cancel
          </button>
        </template>
        ...
        <!-- Unavailable state -->
        <span v-else-if="callState === 'unavailable'" class="text-xs text-slate-400 italic">
          User offline
        </span>

        <!-- Normal call button -->
        <button
          v-else-if="callEnabled && roomId"
          type="button"
...
const props = defineProps<{
  ...
  connected: boolean
  roomId: string | null
  targetUser: PresenceUser | null
  callEnabled?: boolean
  callState?: 'idle' | 'ringing' | 'unavailable'
}>()

const emit = defineEmits<{ send: [text: string]; call: [string]; cancelCall: [] }>()
